...
| `CH_DATA_DIR` | Control Hub JSON root (default `~/control-hub/data`). |

| `CH_API_KEY` | **Strongly recommended** in any shared or remote deployment; required header `X-CH-API-Key` or `Authorization: Bearer` for mutating routes. |
...
Mount real `HERMES_HOME` and `CH_DATA_DIR` volumes that match your agent host, or use bind mounts to host paths.
