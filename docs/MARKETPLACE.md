...
- Generated JSON Schema files live under `packages/schema/json/` (`mission-v1.schema.json`, `template-pack-v1.schema.json`). Regenerate with `npm run generate:json -w @agent-control-hub/schema` from the development monorepo that contains the schema package.
