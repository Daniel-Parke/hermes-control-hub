...
/** Is this a grey? Accent-tinted boundaries are U6's, and are counted not asserted. */
...
test.describe("design invariants", () => {
  for (const route of documentedRoutes()) {
    test(`${route} holds its surfaces apart and its edges visible`, async ({ page }) => {
      ...
      // Anti-vacuity. A collector that returned nothing would pass every
      // assertion below, beautifully.
      expect(census.borders.length, `${route} rendered no boundaries at all`).toBeGreaterThan(5);
