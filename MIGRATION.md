...
2. Or set **`CH_DATA_DIR`** to your existing absolute path (no move required), e.g. in `.env.local`:
   - `CH_DATA_DIR=/home/you/.hermes/control-hub/data`

**Cron repeat:** Recurring jobs created by CH now use **`repeat.times: null`** for “run forever”, matching Hermes’ canonical form (instead of `-1`).
