...
- **Scheduled missions and cron jobs** live in Hermes’ `~/.hermes/cron/jobs.json`. Once written, the **Hermes** process (for example the gateway) **runs** them on its scheduler tick. The Command Hub web app is only an editor for that file plus local dashboard JSON under **`$HOME/command-hub/data/`** (override with **`CH_DATA_DIR`** or **`CONTROL_HUB_DATA_DIR`** so Hermes `mark_job_run` can update mission files in the same place). See [MIGRATION.md](MIGRATION.md) if you used the older default under `~/.hermes/control-hub/data/`.
