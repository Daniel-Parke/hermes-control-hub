// Story Weaver — Prompts (V2 — saved story prompt CRUD)
"use client";
import { useState, useEffect, useCallback } from "react";
import { useRouter } from "next/navigation";
import { ChevronLeft, Plus, X, Save, Trash2, Edit2, FileText, Loader2, ArrowRight } from "lucide-react";
import type { SavedPrompt } from "@/types/recroom";

const EMPTY_PROMPT: Omit<SavedPrompt, "id" | "createdAt" | "updatedAt"> = {
...
export default function PromptsPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (d.data?.prompts) setPrompts(d.data.prompts);
  ...
  const startNew = () => {
    setEditing({ ...EMPTY_PROMPT, id: "", createdAt: "", updatedAt: "" });
    setIsNew(true);
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!isNew) body.promptId = editing.id;
      const res = await fetch("/api/stories", {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const deletePrompt = async (id: string) => {
    ...
    try {
      await fetch("/api/stories", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "prompts", subAction: "delete", promptId: id }),
      });
  ...
  const loadPrompt = (p: SavedPrompt) => {
    router.push(`/recroom/story-weaver/create?promptId=${p.id}`);
  };
    ...
    <div className="min-h-screen bg-dark-950 grid-bg relative scanlines">
        ...
        <div className="fixed inset-0 z-[60] bg-dark-950/80 backdrop-blur-sm flex items-start justify-center p-4 pt-12 overflow-y-auto">
          <div className="bg-dark-900 border border-green-500/20 rounded-xl w-full max-w-2xl p-6 space-y-4 mb-12">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-white">{isNew ? "New Story Prompt" : "Edit Story Prompt"}</h3>
      ...
      <div className="border-b border-white/10 bg-dark-900/50 px-6 py-4 backdrop-blur-xl">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-white">Story Prompts</h1>
          <div className="flex-1" />
          <span className="text-xs text-white/30 font-mono">{prompts.length} prompts</span>
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        {prompts.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-sm text-white/30 mb-2">No saved prompts yet</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {prompts.map(p => (
              <div key={p.id} className="rounded-xl border border-white/5 bg-dark-900/50 p-4 space-y-3 hover:border-white/10 transition-colors">
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-semibold text-white/90 truncate">{p.name}</h3>
                    <p className="text-xs text-white/40 mt-1 line-clamp-3">{p.premise}</p>
                  </div>
                </div>

                {(p.genre.length > 0 || p.era || p.mood.length > 0) && (
                  <div className="flex flex-wrap gap-1">
                    ...
                    {p.era && (
                      <span className="px-1.5 py-0.5 rounded text-[9px] font-mono border border-white/5 bg-white/[0.02] text-white/30">{p.era}</span>
                    )}
